...
export default class ExtraService extends Component {
  ...
  handleSaveData = () => {
      const { selectedExtra, selectedAmenities, selectedUser } = this.state;
      ...
      fetch(BaseUrl.BaseUrl + '/property_add', {
          ...
          .then((data) => {
              if (data.error) {
                  ...
                  alert('Your property has been submitted and is pending approval. You will be notified once it is accepted.');
              }
          })
          .catch((error) => {
